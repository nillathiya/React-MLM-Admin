...
function DirectIncomes() {
  ...
  useEffect(() => {
    const fetchAllDirectIncomeTransactions = async () => {
      try {
        await dispatch(
          getAllIncomeTransactionAsync({ txType: 'income', source: 'direct' }),
        ).unwrap();
      } catch (error: any) {
  ...
  return (
    <div>
      <Breadcrumb pageName="Direct Incomes" />

      <div className="container mx-auto p-6 bg-gray-100 dark:bg-boxdark min-h-screen">
        <h1 className="text-4xl font-extrabold mb-10 text-center text-gray-900 dark:text-gray-100 tracking-wide">
          Direct Incomes
        </h1>
        {incomeTransactionsLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {incomeTransactions.map((transaction: IncomeTransaction) => {
