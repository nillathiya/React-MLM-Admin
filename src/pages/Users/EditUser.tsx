...
const EditUser: React.FC = () => {
  ...
  const validateWalletAddress = (value: string) => {
    // 1. Check if the address is a valid Ethereum address
    const isValidFormat = ethers.isAddress(value); // Using ethers.js
    ...
    // 2. Check if the address is unique
    const isAddressTaken = users.some(
      (user) =>
        user._id !== selectedUser._id &&
        user.walletAddress.toLowerCase() === value.toLowerCase(),
    );
    if (isAddressTaken) {
      return 'This wallet address is already in use.';
    }

    return true; // Valid and unique address
  };
  const {
  ...
  // Fetch user data and update form
  useEffect(() => {
    const fetchUserData = async () => {
      if (!userId) return;

      try {
        // Fetch specific user if not found in users array
        if (!users.find((u) => u._id === userId)) {
          await dispatch(getUserByIdAsync(userId)).unwrap();
        }
        // Fetch all users if user array is empty
        if (users.length === 0) {
  ...
  useEffect(() => {
    if (selectedUser) {
      reset({
        ...
        uSponsor: selectedUser?.uSponsor?._id || '',
                      ...
                      <div className="w-full xl:w-1/2">
                        ...
                        <input
                          ...
                          {...register('walletAddress', {
                            ...
                            validate: validateWalletAddress, // Custom validation
                          })}
                  ...
                  <div className="p-6.5">
                    <div className="mb-4.5 relative">
                      ...
                      <button
                        ...
                        className="absolute right-4 top-12 -translate-y-1/2 text-gray-500 dark:text-gray-400"
                        style={{ border: 'none' }}
                      >
                        {showPassword ? (
